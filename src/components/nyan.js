...
function Nyan() {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} x="0px" y="0px" width={`${width}px`} height={`${height}`} version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" xmlSpace="preserve" style={{backgroundColor:"#000000",zIndex:100}}>
        <defs>
          <g id="rainbow">
        ...
        {stars.map((line,i) => <use x="0" y="0" href="#starburst" transform={`scale(${Math.floor(Math.random()*7)})`} style={{animationStart:`${Math.floor(Math.random()*2)}s`}} key={i}>
            ...
            <animateMotion dur={`${timing}s`} begin={`${Math.random()*i}s`} repeatCount="indefinite">
        ...
        <g transform={`translate(${width/2},${height/2})`}>
         <image href="/images/original_nyan.gif" height="200" width="200" transform="translate(-56,-84)" />
        </g>

        <rect x="0" y="0" width="100px" height="100px" style={{fill:'#000000'}} />
    </svg>
  );
}
